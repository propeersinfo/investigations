...
def write_audio_hash_tag(file, hash):
	try:
		audio = EasyID3(file)
	except ID3NoHeaderError:
		audio = EasyID3()
...
def calc_mp3_audio_hash_mpg123(mp3_file):
	#EXE = 'D:\\portable\\mingw\\msys\\1.0\\home\\Pavel\\mpg123-1.14.4\\doc\\examples\\mp3hash\\mp3hash.exe'
	EXE = 'D:\\portable\\mp3hash.exe'
	#print 'calc_mp3_audio_hash_mpg123: mp3_file = %s %s' % (mp3_file, os.path.exists(mp3_file))
...
def main_update():
	...
	def do_mp3(file):
		...
		if h_tag:
			...
			# if h_tag == h_123:
			# 	#append_file(LOG, 'skipping: as having correct tag set already: %s' % (file))
			# 	pass
			# else:
			# 	append_file(LOG, 'error: file has incorrect tag set: %s' % (file))
		else:
			h_123 = calc_mp3_audio_hash_mpg123(file)
			#h_mpl = calc_mp3_audio_hash_mplayer(file)
			h_mpl = h_123 # '123' code does throw out lame/xing but mplayer does not so not using mplayer anymore
			if h_123 is not None: assert check_hex_digest(h_123)
			if h_mpl is not None: assert check_hex_digest(h_mpl)
			if h_mpl == h_123:
				try:
					...
					# mp3 tagging is dangerous so first we do a test write
					...
					if h_re == h_123:
						...
						append_file(ERROR_LOG, u'ERROR: file got different hash sum after tagging! Original file not affected. %s with %s' % (file, h_re))			
				finally:
					if os.path.exists(tmp):
						os.remove(tmp)
					# print tmp
			else:
				#append_file(BADMP3_LOG, u'skipping: ambiguous hash: %s: %s vs %s' % (file, h_mpl, h_123))
	...
	for dir, subdirs, subfiles in os.walk(unicode(root_dir)):
		for file in glob.glob1(dir, u'*.mp3'):
			try:
				...
				print 'exception: %s' % msg
				append_file(ERROR_LOG, 'exception: %s for file %s' % (msg, file))
...
def main_check_tag():
	...
	print 'total=%d passed=%d failed=%d none=%d' % (stat.cnt_total, stat.cnt_eq, stat.cnt_ne, (stat.cnt_total - stat.cnt_eq - stat.cnt_ne))
